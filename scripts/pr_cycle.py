#!/usr/bin/env python3
"""Safe, explicit PR metadata and isolated test-cycle runner."""
...
def run_test_cycle(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="webgpt-pr-cycle-") as directory:
...
def derive_task_space(state_path: Path) -> str:
    ...
    return f"webgpt-pr-cycle:{state_path.stem}"
...
def start_cycle(
    ...
    *,
    ...
    if not request:
        raise ValueError("cycle request must not be empty")
    if max_iterations < 1:
...
def resume_cycle(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(f"cannot read cycle state: {state_path}") from error
...
def _main() -> int:
    ...
    cycle = subparsers.add_parser("cycle", help="continue one WebGPT/Codex conversation")
    cycle.add_argument("--state", type=Path, required=True)
    cycle.add_argument("--request")
    cycle.add_argument("--pr")
    cycle.add_argument("--commands-json", type=Path)
    cycle.add_argument("--post-comment", action="store_true")
    cycle.add_argument("--new-repo")
    cycle.add_argument("--public", action="store_true")
    cycle.add_argument("--max-iterations", type=int, default=5)
        ...
        if args.action == "status":
            ...
            if args.request:
                ...
                if not args.commands_json:
                    raise ValueError("cycle --pr requires --commands-json")
                result = resume_cycle(
                    args.state,
                    args.pr,
