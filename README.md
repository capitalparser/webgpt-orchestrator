...
2. Before every prompt, the coordinator agent verifies it is in standard Chat—not ChatGPT
   Work or Codex—and selects GPT-5.6 Pro. It then attaches the GitHub connector when it
...
standard Chat with GPT-5.6 Pro. The coordinator verifies the Chat surface and model
...
fallbacks. If standard Chat or GPT-5.6 Pro is unavailable for the current account, the
